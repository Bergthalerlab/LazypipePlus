...
# Converts Kraken2 report file to taxonomy profile
#
# input: nodes.dmp from NCBI taxonomy
#	 names.dmp from NCBI taxonomy containing taxnames output by metaphlan2
#	 clade summary report by: kraken2 --report 
# 
#
# output: taxonomy profile in CAMI format (see https://github.com/bioboxes/rfc/blob/master/data-format/profiling.mkd)
#
# credit: Plyusnin Ilya, University of Helsinki, Finland
# email: Ilja.Pljusnin@helsinki.fi

my $usage=      "USAGE: $0 nodes.dmp names.dmp kraken2.report [-s|sample id] 1> cami_taxonomy_profile\n".
		"\n".
		"nodes.dmp \t\t: node file from NCBI taxonomy\n".
                "names.dmp \t\t: name file from NCBI taxonomy\n".
		"kraken2.report\t\t: kraken2 --report file\n".
		"sample id \t\t: name of the profiled sample\n".
...
GetOptions('help|h' 	=> \$help,
	'sample|s=s'	=> \$sampleid) or die $usage;

if( (scalar @ARGV<3) || $help) { die $usage; }

my $node_dmp_file	= shift(@ARGV);
my $name_dmp_file	= shift(@ARGV);
my $kraken_file		= shift(@ARGV);
...
while(my $l=<IN>){
	...
	if(scalar(@taxpath) < 1){
		next; # no taxpath found for this taxid
	}
